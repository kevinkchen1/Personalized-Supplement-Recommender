...
class DrugBankDataLoader:
    ...
    def load_product_ingredients(self, df: pd.DataFrame):
        ...
        Updated to use MERGE instead of CREATE to prevent duplicate relationships.
        """
        logger.info(f"   Found {len(df):,} product ingredient records")
        logger.info("   Loading in batches to avoid memory issues...")
